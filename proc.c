...
proc_loop(struct tmuxproc *tp, int (*loopcb)(void))
{
	...
	while (!tp->exit && (loopcb == NULL || !(loopcb_ret = loopcb ())));

	printf("loopcb == NULL? %d\n", loopcb == NULL);
	printf("loopcb ret value is? %d\n", loopcb_ret);
	printf("%s loop exit\n", tp->name);
}
...
pid_t
proc_fork_and_daemon(int *fd, int *other_side)
{
	pid_t	pid;
	int	pair[2];
